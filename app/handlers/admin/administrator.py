...
from app.misc.keyboards import main as main_kb, administrator as administrator_kb
...
async def cmd_administrator_cancel(msg: Message, state: FSMContext) -> None:
    ...
    await msg.reply(text=translations[user_language]['messages']['admin']['administrator']['cancel'], reply_markup=await administrator_kb(msg=msg))

for language in languages:
    @router.message(StateFilter(None), IsSubscribedToChannels(), IsRegistered(), IsAdministrator(), F.text.lower() == translations[language]['keyboards']['reply']['admin']['administrator']['main'].lower())
    async def cmd_administrator(msg: Message, state: FSMContext) -> None:
        user_language: str = await get_user_language(telegram_id=msg.from_user.id, language_code=msg.from_user.language_code)
        await state.set_state(state=AdministratorStates.MAIN)
...
for language in languages:
    ...
    async def cmd_administrator_back(msg: Message, state: FSMContext) -> None:
        user_language: str = await get_user_language(telegram_id=msg.from_user.id, language_code=msg.from_user.language_code)
        await state.clear()
        await msg.reply(text=translations[user_language]['messages']['admin']['administrator']['back'], reply_markup=await main_kb(msg=msg))